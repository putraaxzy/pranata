  ...
  return (
    <Card className="border-stone-200 bg-white dark:border-stone-800 dark:bg-stone-900 shadow-sm">
      <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
        ...
        <Link href="/notes" className="text-xs text-stone-900 dark:text-stone-100 hover:underline flex items-center gap-0.5 font-medium">
          View All
          ...
          <div className="space-y-3">
            {notes.map((note) => (
              <Link
                key={note.id}
                href="/notes"
                className="block p-3 rounded-lg border border-stone-100 hover:border-stone-400/40 bg-stone-50/20 hover:bg-stone-50/50 dark:border-stone-800/60 dark:bg-stone-900/20 dark:hover:bg-stone-800/30 transition-all"
              >
                <div className="flex items-center justify-between">
                  <h4 className="text-sm font-semibold text-stone-800 dark:text-stone-100 truncate pr-2">{note.title}</h4>
                  {note.category && (
                    <span className="text-[9px] font-medium text-stone-900 dark:text-stone-100 bg-stone-100 dark:bg-stone-800 px-1.5 py-0.5 rounded capitalize shrink-0">
                      {note.category}
                    </span>
                  )}
                </div>
                ...
                <p className="text-[10px] text-stone-400 mt-2 font-mono" suppressHydrationWarning>
